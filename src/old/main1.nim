...
proc mouseClicked(dt: float32) =
  if mousebtn 0:
    let
      (clickX, clickY) = mouse()
      x = (clickX - borderWidth) div slotWidth mod (size[X] + 1)
      y = (boardHeight() - borderHeight - clickY) div slotHeight mod (size[Y] + 1) - 1
      z = (boardHeight() - borderHeight - clickY) div slotHeight div float(size[Y] + 1)
      w = (clickX - borderWidth) div slotWidth div float(size[X] + 1)
