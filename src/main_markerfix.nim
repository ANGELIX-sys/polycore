...
proc markReached(reached: var Tensor[bool], x, y, z, w: int) =
  # Recursive helper function to mark all cells reachable from the given cell.
  let markerPos = [x, y, z, w]

  reached.atIndex(markerPos) = true
  if x - 1 >= 0:
...
proc generateMaze() =
  ...
  maze = newTensorWith[Cell](size, space)
  
  for c in mitems(maze):
